...
\author{
Hilkka Kontro ${ }^{1}$, Armando Mastracci ${ }^{2}$, Stephen S. Cheung ${ }^{3}$, and Martin J. MacInnis ${ }^{1}$ \\ 1 Faculty of Kinesiology, University of Calgary, Canada \\ 2 Baron Biosystems Ltd., Toronto, Canada \\ 3 Department of Kinesiology, Brock University, Canada
}
...
\section*{Principles of adaptation}
...
\section*{Principles of modeling performance}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-04.jpg?height=553&width=1605&top_left_y=1428&top_left_x=262}
\captionsetup{labelformat=empty}
\caption{Figure 1. The Banister impulse-response model as redrawn from Morton et al. 1990. Performance is a function of fitness and fatigue, which both increase in response to training, but by different magnitudes and following different timeframes.}
\end{figure}

\section*{II. A NOVEL POWER-BASED TRAINING LOAD METRIC}
...
\section*{Popular models for heart rate-derived training load}
...
\section*{III. THREE DIMENSIONS OF TRAINING LOAD}
...
\section*{Rationale for energy-system specific adaptation}
...
\section*{From energy systems to the power-duration relationship}
...
CP has been suggested as the gold standard delineating the heavy intensity domain from the severe intensity domain (Jones et al., 2019). As such, CP is the ceiling for the rate of sustainable oxidative energy provision, as it separates exercise intensities for which a metabolic steady state (i.e., stable oxygen uptake) is achievable from intensities for which it is not (Black et al., 2017; Poole et al., 1988). According to the standard interpretation of the CP model, exercising above CP results in the gradual depletion of W'-the finite work capacity above CP-until task failure occurs, whereas exercising below CP does not draw on $\mathrm{W}^{\prime}$, implying (mathematically) that exercise below CP can be performed indefinitely (Poole et al., 2016). Despite its popularity, this 2-parameter model incorrectly assumes that when duration approaches zero, power output approaches infinity. A 3-parameter (3-CP) power duration-model includes maximal sprint power (Pmax) as a third parameter, addressing the unrealistic assumption of unlimited $\mathrm{W}^{\prime}$ utilization rates (Morton, 1996).

The equation defining the popular 2-parameter power-duration relationship is:
$$
t_{l i m}=\frac{W^{\prime}}{P-C P}
$$

This CP model assumes an unlimited rate of $\mathrm{W}^{\prime}$ expenditure. A 3-parameter (3-CP) power duration-model that includes Pmax as a third parameter has been shown to result in better fit for short exercise durations (Morton, 1996; Vinetti et al., 2019). It is defined as:
$$
\begin{equation*}
t_{l i m}=\frac{W^{\prime}}{P-C P}-\frac{W^{\prime}}{P \max -C P} \tag{3}
\end{equation*}
$$
...
A consequence of the 3-parameter model is that the theoretical Pmax is only achievable in a fatigue-free state (Morton, 1990, 2006). The highest possible power output, maximum power available (MPA), is a function of the remaining $\mathrm{W}^{\prime}$ and decreases with $\mathrm{W}^{\prime}$ expenditure ( $\mathrm{W}^{\prime} \exp$ ). Therefore, MPA changes according to the equation:
$$
\begin{equation*}
M P A=P \max -(P \max -C P) \cdot \frac{W^{\prime} \exp }{W^{\prime}} \tag{4}
\end{equation*}
$$
where $\mathrm{W}^{\prime} \exp =\mathrm{W}^{\prime}-\mathrm{W}^{\prime}$ bal and $\mathrm{W}^{\prime}$ bal is the amount of $\mathrm{W}^{\prime}$ remaining at a given moment. For power outputs $\mathrm{P}>\mathrm{CP}$, task failure occurs when $\mathrm{P}=\mathrm{MPA}$, not when $\mathrm{W}^{\prime}$ is fully depleted.

The three parameters (CP, $\mathrm{W}^{\prime}$, and Pmax ) and the power-duration curve they define can be interpreted to contain information of the three energy systems: their maximal rates and their capacities (for the anaerobic systems, since the aerobic system is not considered capacitylimited) (Morton, 2006). Consequently, a 3-parameter CP model may be used to estimate the contribution of each energy system to the power output at a given moment in time.
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-11.jpg?height=1325&width=1612&top_left_y=244&top_left_x=259}
\captionsetup{labelformat=empty}
\caption{Figure 2. Linking the three energy systems with the three parameters of the power duration relationship. A) The power output attainable for a given duration can be predicted from a 3 parameter critical power (CP) model, in which maximal power output is limited by Pmax, highest aerobically sustainable power output is limited by CP, and the finite work capacity above CP, curvature constant ( $\mathrm{W}^{\prime}$ ), is primary limited by buffering capacity and muscle energy stores available for substrate-level phosphorylation. B) A schematic representation depicting the contribution of each energy system to power production in all-out exercise of 2.5 minutes. The relative contribution of each system is related to the parameters $\mathrm{P}_{\text {max }}$ (the PCr system), $\mathrm{W}^{\prime}$ (the glycolytic system), and CP (the oxidative system). Constructed based on data from Baker et al., (2010), Morton (2006), and Vanhatalo et al., (2007). C) The contribution of the parameters of the 3-parameter CP model to a given power output plotted against power. D) The contribution of the parameters of the 3-parameter CP model to a given power output plotted against the maximal sustainable constant-load duration (D).}
\end{figure}

\section*{Quantification of energy system contributions}
...
At a given moment of time, the power output ( $P$ ) is a sum of energy provision by each energy system. For example, the oxidative system (represented by CP) is unable to provide enough ATP to power 600 W ; therefore, some of the energy must come from the alactic system (Pmax) and the lactic system ( $\mathrm{W}^{\prime}$ ). How much energy comes from each system for each athlete? When an athlete's CP and Pmax are known, we may mathematically derive the contribution of each system to a given $P$. Using the MPA formula (Equation 4), we compute a power duration relationship for a constant power performed to failure in the special case where $P=M P A$ :
$$
\begin{equation*}
P=P \max -(P \max -C P) \cdot \frac{(P-C P) t}{W^{\prime}} \tag{5}
\end{equation*}
$$
...
$$
\begin{equation*}
P=C P+\frac{(P \max -C P) W^{\prime}}{W^{\prime}+(P \max -C P) t} \tag{6}
\end{equation*}
$$

We now have the power-duration formula for the 3-parameter CP model. We can rearrange this formula to compute the contributions of CP , Pmax and $\mathrm{W}^{\prime}$ to power P , as a function of $\mathrm{CP}, \mathrm{W}^{\prime}$, Pmax and P by separating into 3 separate terms:
$$
\begin{equation*}
P=C P+\frac{(P-C P)^{2}}{P \max -C P}+\frac{(P-C P)^{2} t}{W^{\prime}} \tag{7}
\end{equation*}
$$

From this basis, the contributions of CP, Pmax and $\mathrm{W}^{\prime}$ may be derived, assuming $C P$ represents the maximum rate of aerobic power. Therefore:

Contribution of $C P$ :
$$
\begin{equation*}
\text { If } P<C P, P_{C P}=P ; \text { If } P>C P, P_{C P}=C P \tag{8}
\end{equation*}
$$
...
$$
\begin{equation*}
P_{P \max }=\frac{(P-C P)^{2}}{P \max -C P}, \text { where } P>C P \tag{9}
\end{equation*}
$$
...
How can these power allocations be translated into a training load input reflecting the share of each parameter accurately? As argued above, training-induced strain is a function of not only intensity, but also duration. Here, the concept of MPA becomes important, because it allows us to assess how close to their limit the individual is exercising at a given moment of time. When cycling at power output $\mathrm{P}(>\mathrm{CP})$ for long enough to deplete most of $\mathrm{W}^{\prime}$, MPA will approach P , making exercise increasingly difficult and elevating the strain induced by each second of additional exercise. We may estimate the strain coefficient ( $\mathrm{k}_{\text {strain }}$, unitless) by:
$$
\begin{equation*}
k_{\text {strain }}=\frac{P \max -M P A+C P}{P \max -P+C P} \tag{11}
\end{equation*}
$$

Strain rate (SR) is calculated by multiplying $\mathrm{k}_{\text {strain }}$ by power output $(P)$ :
$$
\begin{equation*}
S R=k_{\text {strain }} \cdot P \tag{12}
\end{equation*}
$$
...
$$
\begin{equation*}
S S=\sum S R \cdot\left(\frac{P \max }{C P^{2}} \cdot \frac{100}{3600 s}\right) \tag{13}
\end{equation*}
$$

The factor $P \max / C P^{2}$ is used to achieve $100 S S / \mathrm{h}$ when exercising at $C P$, as can be demonstrated by substituting the equations for $S R$ (12) and $k_{\text {strain }}$ (11) into Equation (13).

This SS can be subdivided into $\mathrm{SS}_{\text {Pmax }}, \mathrm{SS}_{\mathrm{W}^{\prime}}$, and $\mathrm{SS}_{\mathrm{CP}}$ by using the system-specific strain rates $\mathrm{SR}_{\mathrm{CP}}, \mathrm{SR}_{\mathrm{W}^{\prime}}$, and $\mathrm{SR}_{\text {Pmax }}$, calculated from the respective power contributions $\mathrm{P}_{\mathrm{Pmax}}, \mathrm{P}_{\mathrm{W}^{\prime}}$, and $\mathrm{P}_{\mathrm{CP}}$ (Equations 8-10).

For example, if exercising fresh (i.e., W'exp $=0$; see Equation (4)), and power P equals CP, MPA $=$ Pmax and the $\mathrm{k}_{\text {strain }}$ gets a value of 0.27 . However, when available $\mathrm{W}^{\prime}$ is fully exhausted $\left(\mathrm{W}^{\prime} \exp =\mathrm{W}^{\prime}\right), \mathrm{MPA}=\mathrm{CP}$ and $\mathrm{k}_{\text {strain }}$ becomes 1.0. For this example athlete, the model implies that exercising at CP is approximately four times more strenuous when $\mathrm{W}^{\prime}$ has been depleted compared with when they were fresh. The impact of falling MPA on strain is illustrated in Figure 3.

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-15.jpg?height=1148&width=1597&top_left_y=1160&top_left_x=270}
\captionsetup{labelformat=empty}
\caption{Figure 3. Maximum power available (MPA) influences the strain experienced at a given power output. MPA is a function of the amount of W' expended. To generate 600 W when fresh (MPA}
\end{figure}
$=\mathrm{Pmax}=1200 \mathrm{~W}$ ), is less strenuous (grey dot) as compared to a situation where MPA is reduced to 1000,800 , or 600 W (blue, yellow, and red dot, respectively). When MPA $=\mathrm{CP}= 300 \mathrm{~W}, \mathrm{~W}^{\prime}$ is fully depleted and generating 600 W is not possible.

Table 1. Example of the strain score calculation for different levels of MPA

\begin{tabular}{|l|l|l|l|l|l|}
\hline & $\mathrm{MPA}=1200$ & $\mathrm{MPA}=1000$ & $\mathrm{MPA}=800$ & $\mathrm{MPA}=600$ & $\mathrm{MPA}=400$ \\
\hline Power output (W) & & & $\mathbf{k}_{\text {strain }}$ & & \\
\hline 100 & 0.21 & 0.36 & 0.50 & 0.64 & 0.79 \\
\hline 200 & 0.23 & 0.38 & 0.54 & 0.69 & 0.85 \\
\hline 300 & 0.25 & 0.42 & 0.58 & 0.75 & 0.92 \\
\hline 400 & 0.27 & 0.45 & 0.64 & 0.82 & 1.00 \\
\hline 800 & 0.43 & 0.71 & 1.00 & & \\
\hline 1200 & 1.00 & & & & \\
\hline
\end{tabular}

\begin{tabular}{|l|l|l|l|l|l|}
\hline Power output (W) & \multicolumn{5}{|c|}{SR (W)} \\
\hline 100 & 21 & 36 & 50 & 64 & 79 \\
\hline 200 & 46 & 77 & 108 & 138 & 169 \\
\hline 300 & 75 & 125 & 175 & 225 & 275 \\
\hline 400 & 109 & 182 & 255 & 327 & 400 \\
\hline 800 & 343 & 571 & 800 & & \\
\hline 1200 & 1200 & & & & \\
\hline
\end{tabular}

\begin{tabular}{|l|l|l|l|l|l|}
\hline Power output (W) & \multicolumn{5}{|c|}{SS ( $\boldsymbol{\cdot} \mathbf{s}^{\mathbf{- 1}} \boldsymbol{)}$} \\
\hline 100 & 0.008 & 0.013 & 0.019 & 0.024 & 0.029 \\
\hline 200 & 0.017 & 0.028 & 0.040 & 0.051 & 0.063 \\
\hline 300 & 0.028 & 0.046 & 0.065 & 0.083 & 0.102 \\
\hline 400 & 0.040 & 0.067 & 0.094 & 0.121 & 0.148 \\
\hline 800 & 0.127 & 0.212 & 0.296 & & \\
\hline 1200 & 0.444 & & & & \\
\hline
\end{tabular}

\begin{tabular}{|l|l|l|l|l|l|}
\hline Power output (W) & \multicolumn{5}{|c|}{SS ( $\boldsymbol{\cdot} \mathbf{h}^{\mathbf{- 1}} \boldsymbol{)}$} \\
\hline 100 & 29 & 48 & 67 & 86 & 105 \\
\hline 200 & 62 & 103 & 144 & 185 & 226 \\
\hline 300 & 100 & 167 & 233 & 300 & 367 \\
\hline 400 & 145 & 242 & 339 & 436 & 533 \\
\hline 800 & 457 & 762 & 1067 & & \\
\hline 1200 & 1600 & & & & \\
\hline
\end{tabular}

\footnotetext{
Note: Calculated for an athlete whose CP $=300 \mathrm{~W}$ and Pmax $=1200 \mathrm{~W}$
$\mathrm{MPA}=$ Maximum power available; $\mathrm{k}_{\text {strain }}=$ strain coefficient, $\mathrm{SR}=$ strain rate; $\mathrm{SS}=$ strain score.
}

\section*{Advantages of the novel strain score}
...
5 illustrates the effect of quantifying training load using $\mathrm{SS}_{\mathrm{CP}}, \mathrm{SS}_{\mathrm{W}^{\prime}}$, and $\mathrm{SS}_{\text {Pmax }}$ universal, onedimensional metrics. Note that in Figures 4 and 5, the MPA shown follows the modified 3parameter model where the W'exp/ W' factor (see Eq 4) is raised to the $2^{\text {nd }}$ power (Kontro et al., 2024) and the recovery of $\mathrm{W}^{\prime}$ balance follows the differential model by Skiba et al (Skiba et al., 2015) as the combination of these models have shown to effectively predict intermittent exercise in practice (unpublished data).

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-19.jpg?height=1316&width=1638&top_left_y=704&top_left_x=242}
\captionsetup{labelformat=empty}
\caption{Figure 5. A comparison of total work, training stress score (TSS), and strain score (SS) in four different cycling sessions of 10 minutes. Despite the different types of training sessions, the TSS values resulting for $\mathrm{A}, \mathrm{B}$, and D are similar. $\mathrm{SS}_{\mathrm{CP}}, \mathrm{SS}_{\mathrm{W}^{\prime}}$, and $\mathrm{SS}_{\text {Pmax }}$ represent the strain on the aerobic, glycolytic, and PCr energy systems, respectively, which reflects the training load of critical power (CP), work prime (W'), and maximal power (Pmax), respectively. The breakdown of SS into three dimensions allows for a more detailed analysis of the type of training performed. MPA = maximum power available, $\mathrm{CP}=$ critical power; $\mathrm{W}^{\prime}=$ work prime; $\mathrm{Pmax}=$ maximal power output.}
\end{figure}

\section*{IV. THE THREE-DIMENSIONAL IMPULSE-RESPONSE MODEL}
...
$$
\begin{equation*}
p(t)=k_{1} g(t)-k_{2} h(t) \tag{14}
\end{equation*}
$$

Where $p$ is performance response, $t$ is time (in days), $k_{1}$ and $k_{2}$ are weighting factors for fitness and fatigue, respectively, $g(t)$ is fitness, and $h(t)$ is fatigue. The values of $g(t)$ and $h(t)$ depend on the stimulus (i.e., training load), denoted below as $w(t)$, and the time interval, $i$, at which the stimulus is repeated:
$$
\begin{align*}
& g(t)=g(t-i) e^{-i / \tau_{1}}+w(t)  \tag{15}\\
& h(t)=h(t-i) e^{-i / \tau_{2}}+w(t) \tag{16}
\end{align*}
$$

The time constants $\tau_{1}$ and $\tau_{2}$ define the kinetics of fitness and fatigue progression, respectively. An optimal taper for peak performance involves manipulating the training load so that the loss in fitness is minimized while the loss in fatigue is maximized (the effect of varying training load on predicted performance is illustrated in Figure 6).

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-21.jpg?height=1419&width=1623&top_left_y=262&top_left_x=257}
\captionsetup{labelformat=empty}
\caption{Figure 6. Example of predicted progression of fitness (g), fatigue (h), and performance (p) during 100 days of daily training at a constant (A) or variable (B) average training load of $\mathrm{w}(\mathrm{t})=50$ followed by 100 days of detraining at $\mathrm{w}(\mathrm{t})=0$, modelled using equations 14,15 , and 16 . Values for the weighting factors $k_{1}$ and $k_{2}$ are 1 and 1.5 , respectively, and the time constants $\tau_{1}$ and $\tau_{2}$ are 20 days and 7 days, respectively. Units for $\mathrm{g}, \mathrm{h}$, and p on the y -axis are arbitrary. Despite an overall identical accumulated training load of 5000 units over the training period, the performance peak in the variable program (B) is greater than that of the constant program (A).}
\end{figure}
...
$$
g(t)=g(t-1) e^{-1 / \tau_{1}}+w(t)\left(1-e^{-1 / \tau_{1}}\right)
$$

Or, using popularized terminology:
$$
\begin{equation*}
C T L=C T L_{\text {yesterday }} e^{-1 / \tau_{1}}+T S S_{\text {today }}\left(1-e^{-1 / \tau_{1}}\right) \tag{18}
\end{equation*}
$$
...
\section*{Implementation of the 3D-impulse-response model}
...
When calculated in this manner, training load $(\mathrm{w}(\mathrm{t}))$ is divided into three components and fed into three parallel impulse-response models. This way, the three-dimensional model produces three separate performance management charts (PMCs) instead of one. Figure 7 shows a simulation of how fitness, fatigue, and performance of each system could evolve in response to a daily load of $\mathrm{w}(\mathrm{t})=80, \mathrm{w}(\mathrm{t})=18$, and $\mathrm{w}(\mathrm{t})=2\left(\right.$ A.U.) for the oxidative (CP), glycolytic ( $\left.\mathrm{W}^{\prime}\right)$, and PCr (Pmax) systems, respectively.

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/1c15a4a1-2fe7-454e-8adb-88d65c5d1674-23.jpg?height=510&width=1643&top_left_y=236&top_left_x=242}
\captionsetup{labelformat=empty}
\caption{Figure 7. A hypothetical example of how a three-dimensional impulse model would predict the changes in performance for the oxidative system (A), the glycolytic system (B), and the PCr system (C) based on changes in fitness and fatigue for each system. The performance outcome (a.u.) at a given timepoint can be translated into critical power (CP), work prime ( $\mathrm{W}^{\prime}$ ), and maximal power output (Pmax) using a conversion factor to get units of $\mathrm{W}, \mathrm{kJ}$, and W , respectively. Parameter values for the model (weighting factors $\mathrm{k}_{1}$ and $\mathrm{k}_{2}$ and the time constants $\tau_{1}$ and $\tau_{2}$ ) can be experimentally determined for each system. In this simulation, $\mathrm{w}(\mathrm{t})$ is repeated daily and is 80,18 , and 2 , for CP, W', and Pmax, respectively.}
\end{figure}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 2. Reported time constants for fitness ( $\boldsymbol{\tau}_{\mathbf{1}}$ ) and fatigue ( $\boldsymbol{\tau}_{\mathbf{2}}$ ) in endurance sports}
\begin{tabular}{|l|l|l|}
\hline Study & $\boldsymbol{\tau}_{\mathbf{1}}$ & $\boldsymbol{\tau}_{2}$ \\
\hline Busso et al. (2003) (Period 1) & $41 \pm 15$ & $9 \pm 6$ \\
\hline Busso et al. (2003) (Period 2) & $35 \pm 12$ & $13 \pm 3$ \\
\hline Chalencon et al. (2015) (Banister model) & $51 \pm 14$ & $10 \pm 9$ \\
\hline Chalencon et al. (2015) (Variable dose-response model) & $48 \pm 16$ & $8 \pm 4$ \\
\hline Morton et al. (1990) (Subject 1) & 50 & 11 \\
\hline Morton et al. (1990) (Subject 2) & 40 & 11 \\
\hline
\end{tabular}
\end{table}

\section*{V. LIMITATIONS OF THE MODEL}
...
3) The fitness weighing factor (constant $\mathrm{k}_{1}$ ) does not change over time.
...
Baron Biosystems Ltd., 2024 (webpage). MPA ${ }^{\text {TM }}$ - Maximal Power Available https://baronbiosys.com/maximal-power-available/ Accessed Feb 2025.
...
Bouchard, C., Sarzynski, M. A., Rice, T. K., Kraus, W. E., Church, T. S., Sung, Y. J.,...Rankinen, T. (2011). Genomic predictors of the maximal $\mathrm{O}_{2}$ uptake response to standardized exercise training programs. J Appl Physiol (1985), 110(5), 1160-1170. https://doi.org/10.1152/japplphysiol.00973.2010
